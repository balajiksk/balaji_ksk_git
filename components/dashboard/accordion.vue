<template>
     <div class="bg-white rounded-xl shadow-xl overflow-hidden">
          <div v-for="(item, index) in items" :key="index" class="border">
               <button @click="toggleAccordion(index)"
                    ...
                    <Icon v-bind:class="activeIndex === index ? '' : 'rotate-180'"
                         class="h-5 w-5 ease transition-all duration-500" name="mdi:arrow-top-circle" />
               </button>
               <div v-if="activeIndex === index" class="p-4 ease duration-500">
                    {{ item.content }}
               </div>
          </div>
     </div>
